...
#include <stdlib.h>
#define SHADE 196
#define SHADE_RED SHADE								//if color is grayscale keep SHADERED/GREEN/BLUE defined as SHADE and change SHADE
...
#define TOLERANCE_RED TOLERANCE
...
int main(void)
{
	...
	{												//not end of file              :: SEEK_END->From End		  here, but might as well stay
		...
		case 2:										//Color cells in bmp end with red, so logic is applied here. 
			...
			if (((blue >= SHADE_BLUE-TOLERANCE_BLUE)&&(blue <= SHADE_BLUE + TOLERANCE_BLUE))			//if each color is in range of SHADE +- TOLERANCE
				&&((green >= SHADE_GREEN - TOLERANCE_GREEN)&&(green <= SHADE_GREEN + TOLERANCE_GREEN))
